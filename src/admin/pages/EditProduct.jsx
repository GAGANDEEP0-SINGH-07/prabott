...
import { fetchAdminProducts, updateProduct } from '../services/adminApi';
import api from '../../api';
...
export default function EditProduct() {
    ...
    if (!form) return (
        <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4">
            ...
            <button onClick={() => navigate('/admin/products')} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition-colors">Go Back to Products</button>
        </div>
    );
    ...
    return (
        <div className="max-w-3xl space-y-6">
            <div className="flex items-center gap-4 mb-2">
                ...
                <h2 className="text-xl font-bold text-slate-800 tracking-tight">Edit Product</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <Card title="Basic Information">
                    ...
                    <Field label="Description" required>
                        <textarea className="input-field min-h-[100px] resize-y" value={form.description} onChange={e => set('description', e.target.value)} required />
                    </Field>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <Field label="Price ($)" required>
                            <input className="input-field" type="number" min="0" step="0.01" value={form.price} onChange={e => set('price', e.target.value)} required />
                        </Field>
                        <Field label="Stock Quantity" required>
                            <input className="input-field" type="number" min="0" value={form.stock} onChange={e => set('stock', e.target.value)} required />
                        </Field>
                    </div>
                    <Field label="Initial Rating (0–5)">
                        <input className="input-field" type="number" min="0" max="5" step="0.1" value={form.ratings} onChange={e => set('ratings', e.target.value)} />
                    </Field>
                </Card>

                <Card title="Images & Colors">
                    <Field label="Image URLs (one per line)">
                        <textarea className="input-field min-h-[100px] resize-y" value={form.images} onChange={e => set('images', e.target.value)} />
                    </Field>
                    <Field label="Colors (comma-separated)">
                        <input className="input-field" value={form.colors} onChange={e => set('colors', e.target.value)} />
                    </Field>
                </Card>
