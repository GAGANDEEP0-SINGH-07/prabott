...
/* ─────────────────────────── GLOBAL STYLES ─────────────────────────── */
/* Styles extracted to GlobalStyles.css — this component now just ensures the import is loaded */
...
/* ─────────────────────────── CART TOAST ─────────────────────────── */
export function CartToast({ item, visible, message, show }) {
  const isVisible = visible || show;
  const text = message || (item ? `${item.name} added to cart` : '');
  return (
    <div className={`cart-toast ${isVisible ? "show" : ""}`}>
      <span>{"\u2713"}</span>
      <span>{text}</span>
    </div>
  );
